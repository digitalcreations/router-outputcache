...
class ParameterPreparerTest extends PHPUnit_Framework_TestCase {
    function testNoParams() {
        ...
        $arr = $preparer->prepareParameters(['ParameterPreparerHelper', 'noParams'], []);
        $this->assertCount(0, $arr);
    }

    function testBasicVaryBy() {
        $preparer = new \DC\Router\OutputCache\ParameterPreparer(new \DC\Router\OutputCache\Reflector(), []);
        $arr = $preparer->prepareParameters(['ParameterPreparerHelper', 'oneParameterVaries'], ['x' => 1, 'y' => 2]);
    ...
    function testVaryByWithVariationState() {
        ...
        $preparer = new \DC\Router\OutputCache\ParameterPreparer(new \DC\Router\OutputCache\Reflector(),
            [$mockState]);
        $arr = $preparer->prepareParameters(['ParameterPreparerHelper', 'oneState'], []);
        $this->assertCount(1, $arr);
    ...
    function testInvalidStateProvider() {
        $preparer = new \DC\Router\OutputCache\ParameterPreparer(new \DC\Router\OutputCache\Reflector(), []);
        $preparer->prepareParameters(['ParameterPreparerHelper', 'oneState'], []);
    }
}
